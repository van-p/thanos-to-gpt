...
public class GcpHelper
{
    ...
    private static Storage connectToGcp(Config testConfig, String key)
    {
        if (storage == null)
        {
            try
            {
                ...
                {
                    storage = StorageOptions.newBuilder().setCredentials(ServiceAccountCredentials.fromStream(new FileInputStream(CommonUtilities.createFileForEncryptedData(testConfig, System.getProperty("GcpStagingAuthKey"), "gcp-bucket-config.json")))).build().getService();
                }
    ...
    /**
     * This method will use a gcp service account to connect to Client application hosted on GCP. (Security app) and provide a access token which allow access to client apis.
     *
     * @param testConfig             the test config
     * @param applicationIapClientId : Unique client Id for app hosted on google cloud platform.
     * @return : jwt access token, that can be used as bearer auth to access apis.
     */
    public static String connectToAppUsingGcpServiceAccount(Config testConfig, String applicationIapClientId)
    {
        String token = null;
        synchronized (GcpHelper.class)
        {
            token = Config.googleAuthToken;
            if (token == null)
            {
                Credentials credentials;
                try
                {
                    credentials = ServiceAccountCredentials.fromStream(new FileInputStream(CommonUtilities.createFileForEncryptedData(testConfig, System.getProperty("GcpStagingAuthKey"), "gcp-bucket-config.json"))).createScoped();
                    token = ((IdTokenProvider) credentials).idTokenWithAudience(applicationIapClientId, Collections.emptyList()).getTokenValue();
                    Config.googleAuthToken = token;
                } catch (IOException e)
                {
            ...
            {
                testConfig.logComment("Google token found in cache for");
            }
        }
        return token;
    }
    ...
    public static String downloadFileFromGcpBucket(Config testConfig, String authKey, String bucketName, String fileNameInBucket, String localfilePath)
    {
        ...
        CommonUtilities.createFolder(testConfig, localfilePath.substring(0, localfilePath.lastIndexOf(File.separator)));
    ...
    /**
     * Based of flag, renaming of existing files or file deletion can be done. if renaming is used with prefix 'P_' on file testFile.csv, it will become P_testFile.csv
     *
     * @param testConfig            the test config
     * @param authKey               the auth key
     * @param bucketName            the bucket name
    ...
    public static List<String> renameOrDeleteMultipleFiles(Config testConfig, String authKey, String bucketName, List<String> fileNamesListToRename, Boolean wantToRename, String prefixToAppend)
    {
        if (!fileNamesListToRename.isEmpty())
        {
            ...
            } else
            {
                ...
                AssertHelper.compareEquals(testConfig, "Count of file Deleted", fileNamesListToRename.size(), count);
    ...
    public void createAndUploadProcessedFileOnGcp(Config testConfig, String authKey, List<String> downloadFileNames, String commonPathForFileUploadAndDownload, String bucketName)
    {
        ...
        {
            String currentTime = DataGenerator.getTimeinMillSeconds();
